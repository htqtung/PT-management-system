...
class TrainingCalendar extends React.Component {
  ...
  mapEvents = () => {
    ...
    for (let training of this.state.trainings) {
      ...
      item.end = new Date(
        moment(training.date).add(training.duration, 'minutes').format()
      );
      newEvents.push(item);
    }
  ...
  render() {
    ...
    return (
      <div className="container-fluid" style={{ height: 500 }}>
        ...
        />
        <SkyLight
          dialogStyles={smallInfoPopupSkyLight}
        ...
        >
          <div>
            ...
            <div>
              Start at:{' '}
              {moment(clickedEvent.start).format('MMMM Do YYYY, h:mm:ss a')}
            </div>
            <div>
              End at:{' '}
              {moment(clickedEvent.end).format('MMMM Do YYYY, h:mm:ss a')}
            </div>
          </div>
        </SkyLight>
      </div>
